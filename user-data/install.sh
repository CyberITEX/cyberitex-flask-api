...
# Write content to a file (via sudo) only if it differs from the current
# contents. Skips (and logs) when already up to date, so re-runs are no-ops.
# Usage: write_file_if_changed <dest> <content> [owner:group] [mode]
write_file_if_changed() {
    ...
    if [[ -f "$dest" ]] && [[ "$(sudo cat "$dest" 2>/dev/null)" == "$content" ]]; then
        log "$dest already configured, skipping"
        return 0
    fi
...
configure_sysctl() {
    ...
    # Dedicated sysctl drop-in (avoids polluting main /etc/sysctl.conf)
    local content
    content=$(cat <<'EOF'
...
EOF
    ...
    # Apply settings (log_warn covers keys whose modules aren't loaded, e.g. bbr)
...
setup_unattended_upgrades() {
    SETUP_PHASE="unattended_upgrades"
    log "Configuring unattended-upgrades"

    if ! dpkg -s unattended-upgrades &>/dev/null; then
        log "Installing unattended-upgrades"
        sudo apt-get update -y
        sudo apt-get install -y unattended-upgrades || error_exit "Failed to install unattended-upgrades"
    else
        log "unattended-upgrades already installed"
    fi

    # Note: ${distro_id}/${distro_codename} are apt template vars, not shell —
    # the quoted heredoc keeps them literal.
    local uu_conf
    uu_conf=$(cat <<'EOF'
...
EOF
    ...
    write_file_if_changed "/etc/apt/apt.conf.d/50unattended-upgrades" "$uu_conf"
...
EOF
    ...
    write_file_if_changed "/etc/apt/apt.conf.d/20auto-upgrades" "$auto_conf"

    sudo systemctl enable unattended-upgrades &>/dev/null || true
    if ! sudo systemctl is-active --quiet unattended-upgrades; then
        sudo systemctl start unattended-upgrades || log_warn "Could not start unattended-upgrades"
    fi
...
setup_auto_reboot() {
    SETUP_PHASE="auto_reboot"
    log "Configuring weekly reboot-if-required cron"

    local cron_content
    cron_content=$(cat <<'EOF'
# Reboot if required - Saturday 5 AM New York (10:00 UTC)
0 10 * * 6 root [ -f /var/run/reboot-required ] && /sbin/reboot
EOF
)
    write_file_if_changed "/etc/cron.d/weekly-reboot" "$cron_content" "root:root" "644"

    log "Weekly reboot cron configured"
}
...
setup_auditd() {
    ...
    if ! dpkg -s auditd &>/dev/null; then
        ...
        sudo apt-get install -y auditd || error_exit "Failed to install auditd"
    else
        log "auditd already installed"
    fi

    sudo systemctl enable --now auditd &>/dev/null || log_warn "Could not enable/start auditd"
...
setup_docker() {
    ...
    if command -v docker &>/dev/null; then
        ...
        curl -fsSL https://get.docker.com | sudo sh || error_exit "Docker installation failed"
        sudo systemctl enable docker &>/dev/null || true
        log "Docker installed"
    fi

    # Allow the service user to run docker without sudo (idempotent)
    if [[ "$CUSTOM_USER" != "root" ]] && id "$CUSTOM_USER" &>/dev/null; then
        if ! id -nG "$CUSTOM_USER" | tr ' ' '\n' | grep -qx docker; then
            sudo usermod -aG docker "$CUSTOM_USER" || log_warn "Could not add $CUSTOM_USER to docker group"
            log "Added $CUSTOM_USER to docker group (re-login required to take effect)"
        fi
    fi
}

setup_vscode_server() {
    SETUP_PHASE="vscode_server"
    log "Configuring VSCode Server file-scanning limits"

    local settings_file="$USER_HOME/.vscode-server/data/Machine/settings.json"
...
EOF
    ...
    local owner=""
    [[ "$CUSTOM_USER" != "root" ]] && owner="$CUSTOM_USER:$CUSTOM_USER"
    write_file_if_changed "$settings_file" "$settings" "$owner"

    # write_file_if_changed created parent dirs as root; hand the tree back to the user
    if [[ "$CUSTOM_USER" != "root" ]]; then
        sudo chown -R "$CUSTOM_USER:$CUSTOM_USER" "$USER_HOME/.vscode-server" 2>/dev/null || true
    fi

    log "VSCode Server settings configured at $settings_file"
}
...
main() {
    ...
    # Phase 2b: System Hardening & Maintenance
    setup_unattended_upgrades
    setup_auto_reboot
    setup_auditd
